 ...
 * Adapted from Go net/rpc/server.go.
   ...
   ** end.Call("Raft.AppendEntries", &args, &reply) -- send an RPC, wait for reply.
	...
	-> The "AppendEntries" is the name of the method to be called.Call() returns true
	to indicate that the server executed the requestand the reply is valid.
	...
	if the handler function on the server side does not return. That is, there
   ...
   ** srv := MakeServer()
   ** srv.AddService(svc) -- a server can have multiple services, e.g. Raft and k/v pass srv to net.AddServer()
   ** svc := MakeService(receiverObject) -- obj's methods will handle RPCs much like Go's rpcs.Register()
 ...
 * A server is a collection of services, all sharing 
 the same rpc dispatcher. so that e.g. both a Raft and a k/v server can listen to the same rpc endpoint.
 */
