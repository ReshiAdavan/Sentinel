 ...
 * Allocate new outgoing port file names, and a new state persister, 
 to isolate previous instance of this server. 
 ...
 * Indirectly checks that the servers agree on the same value, 
 since nCommitted() checks this, as do the threads that read from applyCh.
 * Returns index.
 * If retry is true, may submit the command multiple times, in case a leader fails just after Start().
